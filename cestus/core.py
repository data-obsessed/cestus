...
class CestusReader:
    ...
    def _decode_column(encoded_data, dt_column):
        match dt_column:
            case float():
                return [
                    struct.unpack("f", encoded_data[i : i + 4])[0]
            ...
            case bool():
                return [
                    struct.unpack("?", encoded_data[i : i + 1])[0]
                    for i in range(0, len(encoded_data), 1)
                ]
            case int():
                return [
                    struct.unpack("i", encoded_data[i : i + 4])[0]
                    for i in range(0, len(encoded_data), 4)
                ]
            case _:
...
class CestusWriter:
    ...
    def _calc_stats(self, column: List[str], column_name):
        ...
        for name, np_function in zip(stats_name, np_function):
            self._stats[column_name][name] = np_function(
                [float(v) if v != "" else 0.0 for v in column]
            ).tolist()
    ...
    def _encode_column(self, column, column_dt, column_name):
        ...
        match column_dt:
            ...
            case float():
                return b"".join(
                    struct.pack("f", float(v) if v != "" else 0.0) for v in column
                )
            case str():
